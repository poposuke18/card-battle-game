...
export const TURN2_CARDS: Card[] = [
  ...
  createCard({
    ...
    effect: {
      type: 'UNIT_VERTICAL_ENEMY_DEBUFF',
      power: Math.round((EFFECT_VALUES.VERTICAL * 1.5) / 10) * 10 
    },
    turn: 2
  }),
  createCard({
    ...
    effect: {
      type: 'SELF_POWER_UP_BY_ADJACENT_ALLY',
      power: Math.round((EFFECT_VALUES.ADJACENT * 1.5) / 10) * 10
    },
    turn: 2
  }),
  createCard({
    ...
    effect: {
      type: 'DIAGONAL_DEBUFF',
      power: Math.round((EFFECT_VALUES.DIAGONAL * 1.5) / 10) * 10
    },
    turn: 2
  }),
  createCard({
