...
export const TURN5_CARDS: Card[] = [
  ...
  createCard({
    ...
    effect: {
      ...
      power: Math.round((EFFECT_VALUES.ADJACENT * 1.5) / 5) * 5,
      range: 1
    },
    turn: 5
  }),
  createCard({
    ...
    effect: {
      ...
      range: 2,
      allyBonus: Math.round((EFFECT_VALUES.FIELD * 0.8) / 5) * 5,
      enemyPenalty: -Math.round((EFFECT_VALUES.FIELD * 0.6) / 5) * 5
    },
    turn: 5
  }),
  createCard({
  ...
  createCard({
    ...
    effect: {
      ...
      power: Math.round((EFFECT_VALUES.VERTICAL * 0.5) / 5) * 5,
  ...
  createCard({
    ...
    effect: {
      type: 'ROW_COLUMN_BUFF',
      power: Math.round((EFFECT_VALUES.HORIZONTAL * 0.5) / 5) * 5,
  ...
  createCard({
    ...
    effect: {
      type: 'ADJACENT_UNIT_BUFF',
      power: Math.round((EFFECT_VALUES.ADJACENT * 2) / 5) * 5,
      range: 1
    },
    turn: 5
  }),
  ...
  createCard({
    ...
    effect: {
      type: 'ROW_COLUMN_BUFF',
      power: Math.round((EFFECT_VALUES.VERTICAL * 0.8) / 10) * 10,
      targetDirection: 'vertical'
    },
    turn: 5
  }),
  createCard({
    ...
    effect: {
      type: 'ROW_COLUMN_BUFF',
      power: Math.round((EFFECT_VALUES.HORIZONTAL * 0.8) / 10) * 10,
      targetDirection: 'horizontal'
    },
    turn: 5
  })
