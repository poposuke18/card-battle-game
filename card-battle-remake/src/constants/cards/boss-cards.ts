...
function calculateBossPoints(basePoint: number, stage: number): number {
  ...
  const rawPoints = basePoint * bossMultiplier * stageMultiplier;
...
const STAGE1_BOSS: Card = createCard({
  ...
  effect: {
    ...
    primaryEffect: {
      range: 2,
      enemyPenalty: -Math.round((EFFECT_VALUES.FIELD * 1.5) / 5) * 5,
...
const STAGE2_BOSS: Card = createCard({
  ...
  effect: {
    ...
    primaryEffect: {
      range: 3,
      pattern: 'cross',
      enemyPenalty: -Math.round((EFFECT_VALUES.FIELD * 2) / 5) * 5,
      weaponNullification: true,
      description: '十字方向3マスの敵を弱体化し、武器効果を無効化'
    },
    secondaryEffect: {
      allyBoost: Math.round((EFFECT_VALUES.FIELD * 1.5) / 5) * 5,
      targetDirection: 'horizontal',
      description: '同じ列の味方を強化'
    },
    ultimateEffect: {
      name: 'メガフレア',
      damage: Math.round((EFFECT_VALUES.FIELD * 2) / 5) * 5,
      pattern: 'all',
...
const STAGE3_BOSS: Card = createCard({
  ...
  effect: {
    ...
    ultimateEffect: {
      name: 'TSUNAMI',
      power: Math.round((EFFECT_VALUES.FIELD * 3) / 5) * 5,
      targetDirection: 'all',
...
export const TURN8_CARDS = [STAGE1_BOSS, STAGE2_BOSS, STAGE3_BOSS, STAGE4_BOSS];
